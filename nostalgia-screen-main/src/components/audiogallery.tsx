...
import { useAudioPlayer } from '../hooks/useAudioPlayer'; // Custom hook for audio control
...
export default function AudioGallery() {
  ...
  return (
    <motion.div
      initial={{ opacity: 0 }}
      animate={{ opacity: 1 }}
      className="min-h-screen w-full bg-background text-foreground px-4 py-8 md:px-8"
    >
      <header className="mb-8">
        <div className="flex items-center gap-4">
          ...
          <h1 className="text-foreground font-serif text-4xl font-bold">Sonic Gallery</h1>
        </div>
      </header>
      ...
      >
        {mockAudios.map((audio) => (
          <motion.div
            ...
            className="relative group bg-card/50 p-6 rounded-2xl cursor-pointer hover:shadow-2xl transition-all border border-border"
            ...
            <div className="relative z-10">
              ...
              >
                <div className="relative w-16 h-16 flex items-center justify-center">
                  <Disc className="w-12 h-12 text-muted-foreground/30 animate-spin-slow" />
                  <Volume2 className="absolute w-6 h-6 text-primary" />
                </div>
                <div>
                  <h3 className="text-foreground text-xl font-medium">{audio.title}</h3>
              ...
              <div className="mt-4 h-12 bg-background/30 rounded-lg p-2">
        ...
        {selectedAudio && (
          ...
          >
            <motion.div
              ...
              className="max-w-4xl w-full bg-card rounded-3xl shadow-2xl overflow-hidden border border-border"
              onClick={(e) => e.stopPropagation()}
            >
              <div className="p-8 space-y-6">
                <div className="flex justify-between items-start">
                  <div>
                    <h2 className="text-3xl text-foreground font-serif">
                      {selectedAudio.title}
                    </h2>
                    <p className="text-muted-foreground">{selectedAudio.date}</p>
                  </div>
                  <button
                    onClick={() => setSelectedAudio(null)}
                ...
                <div className="relative h-48 bg-background/30 rounded-xl overflow-hidden">
                  <div className="absolute inset-0 flex items-center justify-center">
                    <Waves className="w-24 h-24 text-muted-foreground/10" />
                  </div>
                ...
                <div className="space-y-4">
                  <div className="h-1 bg-muted-foreground/10 rounded-full">
                    <motion.div
